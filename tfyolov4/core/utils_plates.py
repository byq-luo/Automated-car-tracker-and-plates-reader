...
def get_boxes_coords(image, bboxes, box_no, classes=read_class_names(cfg.YOLO.CLASSES)):
    ''' unpacks selectted box coordinates'''

    out_boxes, out_scores, out_classes, num_boxes = bboxes
    num_classes = len(classes)

    image_h, image_w, _ = image.shape
        # TODO find out what is the purpose of next line check
    if int(out_classes[0][box_no]) < 0 or int(out_classes[0][box_no]) > num_classes:
        print('Check for get_boxes_coords not passed. Zero coords returned')
        return [0, 0, 0, 0]
    coor = out_boxes[0][box_no]
    coor[0] = int(coor[0] * image_h)
    coor[2] = int(coor[2] * image_h)
    coor[1] = int(coor[1] * image_w)
    coor[3] = int(coor[3] * image_w)

    print('[INFO] coor unpacked')
    return coor
